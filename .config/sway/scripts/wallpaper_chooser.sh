...
if [ -n "$selected" ]; then
    ...
    if [[ "$thumbnail_path" == "$SHUFFLE_ICON" ]]; then
        ...
        original_path=$(find "$WALLPAPER_DIR" -type f | shuf -n 1)
    else
    ...
    if [ -n "$original_path" ]; then
        ...
        notify-send -t 2000 "Wallpaper" "Wallpaper has been updated" -i "$original_path"
    else
        notify-send 2000 "Wallpaper Error" "Could not find the original wallpaper file."
    fi
fi
